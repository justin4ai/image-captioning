...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pos = torch.arange(0, max_len).view(max_len, 1)
        print(pos.size()) # torch.Size([5000, 1])


        t = torch.pow(torch.tensor([(1/ (10)**4)]), torch.arange(0, embed_dim, 2) / embed_dim).view(1, -1)
        print(t.size()) # torch.Size([1, 3]) 
