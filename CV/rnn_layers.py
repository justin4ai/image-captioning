...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h_no_act = x @ Wx + prev_h @ Wh + b
...
def rnn_step_backward(dnext_h, cache):
    ...
    dh = 1 - np.tanh(next_h_no_act) ** 2
    dtanh =  dh * dnext_h # f'(tanh) -> 1 - (tanh)^2
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(x.shape[1]):
        ...
        x_t = x[:, t, :] 
        h_prev = h[:, t - 1, :]
        h[:, t, :], cache_step = rnn_step_forward(x_t, h0, Wx, Wh, b) if t == 0 else rnn_step_forward(x_t, h_prev, Wx, Wh, b) 
        cache_steps.append(cache_step) # from 0 to T-1
...
def rnn_backward(dh, cache):
    ...
    dx = np.zeros((N, T, D)) # T is for stacking
    db = np.zeros((H,))
    dWx = np.zeros((D, H))
    dWh = np.zeros((H, H))
    dh_prev = 0
    ...
    for t in reversed_timespace:
        ...
        dh_t, cache_t = dh[:, t, :], cache_steps[t]
        dh_cum = dh_t + dh_prev

        dx_t, dh_prev, dWx_t, dWh_t, db_t = rnn_step_backward(dh_cum, cache_t)
        
        dx[:, t, :] = dx_t # so I stack
        db = db + db_t
        dWx = np.add(dWx, dWx_t)
        dWh = np.add(dWh, dWh_t)
...
def word_embedding_forward(x, W):
    ...
    out, cache = W[x], (x, W) # Followed the hint!
...
def word_embedding_backward(dout, cache):
    ...
    for i in range(N):
        for j in range(T):
            ...
            idx, upstream_gradient = x[i, j], dout[i, j]
            np.add.at(dW, idx, upstream_gradient) # followed the hint
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    do = dnext_h * np.tanh(next_c) # f'(o * tanh(next_c)) by o -> tanh(next_c)
    dc = dnext_h * o * (1 - np.tanh(next_c) ** 2) # f'(o * tanh(next_c)) by c -> o * (1 - tanh(next_c)^2)
    
    dc += dnext_c # dc calculated (two ways)
    
    # go one level deeper - dc * f'(c_t) by ? 
    df = dc * prev_c
    dprev_c = dc * f
    di = dc * g  
    dg = dc * i

    # go one level deeper - d? * f'(activation) by ??      
    da_g = dg * (1 - g ** 2)
    da_i = di * (1 - i) * i
    da_f = df * (1 - f) * f
    da_o = do * (1 - o) * o
