...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # (1) Use an affine transformation to compute the initial hidden state     #
        #     from the image features. This should produce an array of shape (N, H)
        (h_0, cache_affine) = affine_forward(features, W_proj, b_proj)

        # (2) Use a word embedding layer to transform the words in captions_in     #
        #     from indices to vectors, giving an array of shape (N, T, W).         #
        (word_emb, cache_emb) = word_embedding_forward(captions_in, W_embed) # I need no cache

        # (3) Use either a vanilla RNN or LSTM (depending on self.cell_type) to    #
        #     process the sequence of input word vectors and produce hidden state  #

        (h, cache_rnn) = rnn_forward(word_emb, h_0, Wx, Wh, b) if self.cell_type =='rnn' else lstm_forward(word_emb, h_0, Wx, Wh, b)

        # (4) Use a (temporal) affine transformation to compute scores over the    #
        #     vocabulary at every timestep using the hidden states, giving an      #
        #     array of shape (N, T, V).                                            #
      
        (scores, cache_tmp) = temporal_affine_forward(h, W_vocab, b_vocab)
